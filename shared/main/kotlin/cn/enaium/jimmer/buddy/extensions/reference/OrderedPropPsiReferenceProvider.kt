 ...
 * @author Enaium
 */
object OrderedPropPsiReferenceProvider : PsiReferenceProvider() {
    ...
    private class Reference(e: PsiElement) : PsiReferenceBase<PsiElement>(e) {
        ...
        private fun getProps(element: PsiElement): Map<String, PsiElement> {
            if (element.annotName() != OrderedProp::class.qualifiedName) {
                return emptyMap()
            }
            ...
            element.getParentOfType<PsiMethod>(true)?.getTarget()?.methods?.forEach { method ->
                result[method.name] = method
            }

            element.getParentOfType<KtProperty>(true)?.getTarget()?.getProperties()?.forEach { property ->
