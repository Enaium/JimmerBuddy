...
class JdbcDatabaseMetadataProvider : DatabaseMetadataProvider {
    ...
    override fun getTables(project: Project, item: DatabaseCache.DatabaseItem): Set<Table> {
        ...
        var driverJarFile = item.driverFile.takeIf { it.isNotBlank() }?.let { Path(it).takeIf { path -> path.exists() } }
        val driverName = item.driverName.takeIf { it.isNotBlank() }
        ...
        if (driverJarFile == null && jdbcDriver != null) {
            ReadAction.compute<Array<com.intellij.openapi.vfs.VirtualFile>, Throwable> {
                OrderEnumerator.orderEntries(project).runtimeOnly().classesRoots
            }.forEach {
        ...
        if (driverJarFile == null) {
            ...
            } else if (item.driverFile.isNotBlank()) {
        ...
        return getConnection(item).use { connection ->
            connection.metaData.getTables(
                catalog = item.catalog.takeIf { it.isNotBlank() },
                schemaPattern = if (isDDL) "public" else item.schemaPattern.takeIf { it.isNotBlank() },
                tableNamePattern = item.tableNamePattern.takeIf { it.isNotBlank() },
            )
        }
    }

    private fun getConnection(item: DatabaseCache.DatabaseItem): Connection {
        return URI.create(item.uri.replace("\\", "/")).takeIf { it.scheme == "file" }?.let { ddl ->
            ...
            DriverManager.getConnection(item.uri, item.username, item.password)
        }
    }
