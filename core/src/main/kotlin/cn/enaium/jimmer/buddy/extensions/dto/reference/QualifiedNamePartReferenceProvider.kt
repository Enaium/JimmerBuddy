    ...
    class QualifiedNamePartReference(
        ...
        override fun resolve(): PsiElement? {
            ...
            if (isLastPart) {
                ...
                resolveClass(fullQualifiedName)?.let { return it }
                // If direct lookup fails, try to resolve through import statements
                if (parts.size == 1) {
                    return resolveThroughImport(fullQualifiedName)
                }
                return null
            } else {
        ...
        private fun resolveThroughImport(shortName: String): PsiElement? {
            val file = qualifiedNamePart.containingFile as? DtoPsiFile ?: return null
            ...
            for (importStatement in importStatements) {
                val importParts = importStatement.qualifiedName.qualifiedNamePartList.map { it.identifier.text }
                if (importParts.lastOrNull() == shortName) {
                    val importFullName = importParts.joinToString(".")
                    resolveClass(importFullName)?.let { return it }
                }
            }
            return null
        }
